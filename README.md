...
- **ضبط چندصفحه‌ای** — اگر بعد از یک اکشن به صفحه/لینک دیگری رفتی، ضبط ادامه پیدا می‌کند (navigate خودکار ثبت می‌شود)
- **کپچا / ورود دستی** — فیلد کپچا خودکار تشخیص داده می‌شود؛ هنگام اجرا برنامه مکث می‌کند تا خودت کد را وارد کنی
...
- **ذخیرهٔ سشن کامل** — کوکی + `localStorage` + `sessionStorage` ذخیره می‌شود تا لاگین موقع اجرا حفظ شود
- **مرورگر پایدار** — یک مرورگر برای همهٔ اجراها باز می‌ماند و بین ردیف‌ها دوباره لاگین نمی‌کند
...
```json
{
  ...
  "actions": [
    ...
    },
    {
      ...
      "value": "{ASK}",
      "captcha": true,
      "description": "کد امنیتی (هنگام اجرا دستی)"
    },
    {
      "step": 4,
      "type": "keyboard",
...
|-----|----------------|-------|
| `click` | `xpath` | کلیک روی المان — اگر روی `svg`/آیکون کلیک کنی، خودکار دکمهٔ والد کلیک می‌شود. اگر `value` داشت بعد از کلیک تایپ می‌کند |
| `input` | `xpath`, `value` | تایپ در فیلد (input, textarea, contenteditable) |
| `manual` | `xpath` | مکث می‌کند و در ترمینال از تو می‌پرسد (برای کپچا). مقدار `{ASK}` |
| `keyboard` | `key` | فشردن کلید (`Return`, `Tab`, `Escape`, `ArrowDown`, ...) |
| `navigate` | `url` | رفتن به آدرس جدید |
| `wait` | `seconds` | صبر کردن |

> **کپچا:** هر فیلد `input` که در `xpath`/توضیحش کلمهٔ `captcha` یا «کد امنیتی» باشد، حتی اگر `manual` نباشد، خودکار به حالت ورود دستی می‌رود (مقدار ضبط‌شده دوباره تایپ نمی‌شود چون هر بار عوض می‌شود).
...
| `--headless` | ❌ | `false` | اجرا بدون پنجره مرورگر (با کپچا/دستی سازگار نیست) |
| `--delay` | ❌ | `0.4` | تاخیر بین اکشن‌ها (ثانیه) |
| `--keep-open` | ❌ | `false` | بعد از پایان، مرورگر را باز نگه دار (تا Enter بزنی) |
| `--fresh-session` | ❌ | `false` | قبل از هر ردیف، دوباره کوکی/سشن را اعمال کن (پیش‌فرض: فقط یک‌بار) |

> **توجه:** برنامه یک مرورگر را برای همهٔ ردیف‌ها باز می‌کند، سشن (کوکی + storage) را **یک‌بار** بازمی‌گرداند و بین ردیف‌ها دوباره لاگین نمی‌کند. برای فرم‌هایی که باید هر بار از نو پر شوند، بین ردیف‌ها به `url` اول برمی‌گردد.
...
- موقع **توقف ضبط**، افزونه خودکار کوکی‌ها + `localStorage` + `sessionStorage` صفحه را ذخیره می‌کند (برای سایت‌هایی که توکن لاگین را در storage نگه می‌دارند، نه کوکی)
- کوکی‌های لاگین معمولاً **۷ تا ۳۰ روز** اعتبار دارند؛ بعد از انقضا دوباره ضبط کن
- برخی سایت‌ها از **CSRF token** استفاده می‌کنند — اگر اتوماسیون کار نکرد، اول با کوکی‌ها صفحه را باز کن

### درباره کپچا

- اگر صفحه کپچا داشته باشد، **نمی‌توان** کد را از پیش ضبط کرد چون هر بار عوض می‌شود
- افزونه فیلد کپچا را تشخیص می‌دهد و به‌جای مقدار ثابت، `{ASK}` می‌گذارد
- هنگام اجرا (بدون `--headless`)، برنامه روی فیلد کپچا **مکث می‌کند**؛ کد را از پنجرهٔ مرورگر بخوان و در ترمینال تایپ کن و Enter بزن
- بهترین راه برای دور زدن کامل کپچا: یک‌بار دستی لاگین کن، کوکی/سشن را ذخیره کن و اجراهای بعدی با همان سشن وارد می‌شوند
